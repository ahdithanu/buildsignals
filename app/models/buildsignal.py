...
class BuildSignalPublication(OrgMixin, Base):
    __tablename__ = "buildsignal_publications"
    __table_args__ = (UniqueConstraint("revision_id", "version", name="uq_publication_revision_version"),)

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    revision_id: Mapped[str] = mapped_column(String(36), ForeignKey("buildsignal_revisions.id", ondelete="CASCADE"), index=True)
    actor_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
    review_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("buildsignal_reviews.id", ondelete="SET NULL"))
