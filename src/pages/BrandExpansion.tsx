...
import { ArrowRight, Building2, Map, Radar, RefreshCw } from 'lucide-react';
...
export default function BrandExpansion() {
  ...
  const { data, isLoading, isFetching, error, refetch } = useBrandExpansion(days);
  const rows = data ?? [];
  ...
  return (
    <Layout>
      <main className="mx-auto w-full max-w-[1500px] p-4 pb-16 md:p-6">
        <header className="flex flex-col justify-between gap-3 border-b-2 border-foreground pb-4 sm:flex-row sm:items-end">
          <div>
            <div className="flex items-center gap-2">
              <Building2 className="h-5 w-5" />
              <h1 className="font-display text-xl font-semibold">Brand Expansion</h1>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Chain activity detected in permit filings, ranked with nearby acquisition candidates
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        {isLoading && <LoadingState message="Ranking brand expansion signals..." />}
        {error && <ErrorState message="Brand expansion intelligence could not be loaded." onRetry={() => refetch()} />}
        {!isLoading && !error && rows.length === 0 && (
          <EmptyState
            title="No active brand signals"
            description={`No candidate or confirmed chain activity was observed in the last ${days} days.`}
        ...
        {!isLoading && !error && rows.length > 0 && (
          <>
            <section className="grid grid-cols-2 border-b-2 border-foreground sm:grid-cols-4" aria-label="Expansion totals">
              <Metric label="Brand signals" value={totals.signals} />
            ...
            <section className="divide-y-2 divide-foreground border-b-2 border-foreground" aria-label="Top expanding brands">
              {rows.map((row, index) => (
                <article key={row.brand.id} className="grid gap-3 bg-card px-3 py-4 lg:grid-cols-[42px_1.1fr_1.4fr_180px] lg:items-center lg:px-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-baseline gap-x-2">
                      ...
                      <span className="text-[10px] text-muted-foreground">{row.brand.category || 'Chain'}</span>
                    </div>
